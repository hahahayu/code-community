<template>
    <div class="container">
      <!-- 固定导航 -->
      <van-nav-bar fixed left-arrow @click-left="$router.back()" title="小鱼同学"></van-nav-bar>

      <!-- 聊天主体区域 -->
      <div class="chat-list">
        <div v-for="(obj, index) in list" :key="index">
            <!-- 左侧是机器人小鱼 -->
            <div class="chat-item left" v-if="obj.name === 'xy'">
...
export default {
  ...
  data () {
    return {
      ...
      list: [
        ...
        { name: 'xy', msg: 'hi，你好！我是小鱼' }
        // { name: 'me', msg: '我是编程小王子' }
      ],
      socket: null // 客户端和服务端建立链接的socket对象
    }
  },
  created () {
    // 创建客户端 websocket 的实例
  ...
  methods: {
    ...
    sendFn () {
      ...
      // 客户端调用 socket.emit('message', 消息内容) 方法把消息发送给 websocket 服务器：
